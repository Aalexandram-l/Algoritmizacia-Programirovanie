/* Класс люди, которые харакетризуются полями ФИО, год рождения. 2 наследника: 
  1) класс студенты, они характеризуются одномерным массивом оценок
  2) прподаватели - массив наименований предметов, которые они ведут. 
Программа котора реализована в меню: 
1) заполнение
2) печать данных
3) выборка студентов по году рождения
4) выборка преподователей по наименованию дисциплины 
5) Выход. 
Нельзя делать выборку если данные не занесены. данные подаются корректно*/
/*
